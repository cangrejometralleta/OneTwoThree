...
func TestConfigPrecedence(t *testing.T) {
	...
	path := filepath.Join(root, "config", "order.production.json")
	...
	config, err := LoadOrderConfig(root, environment)
	if err != nil {
		t.Fatal(err)
	}
	...
	if ReadOrderConstants().ItemNameWidth != 12 {
...
func TestConfigRejectsInvalidInputs(t *testing.T) {
	cases := []struct {
		...
		{name: "empty override", environment: map[string]string{"ORDER_PORT": ""}},
		{name: "invalid port", environment: map[string]string{"ORDER_PORT": "65536"}},
		{name: "fractional port", environment: map[string]string{"ORDER_PORT": "3.5"}},
		{name: "empty path", environment: map[string]string{"ORDER_DATABASE_PATH": " "}},
		{name: "global variable", environment: map[string]string{"ORDER_ITEM_NAME_WIDTH": "1"}},
		{name: "unknown variable", environment: map[string]string{"ORDER_TYPO": "1"}},
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := LoadOrderConfig(root, environment); err == nil {
...
func TestConfigValidatesBeforeOverrides(t *testing.T) {
	root := buildConfigFixture(t)
	path := filepath.Join(root, "config", "order.defaults.json")
	if err := os.WriteFile(path, []byte(`{"port":null}`), 0600); err != nil {
		t.Fatal(err)
	}

	_, err := LoadOrderConfig(root, map[string]string{"APP_ENV": "production", "ORDER_PORT": "9100"})
...
func TestGlobalConstantsRejectInvalidData(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{"itemNameWidth":null}`), 0600); err != nil {
		t.Fatal(err)
	}
	defer func() {
		if recover() == nil {
	...
	readGlobalValues[map[string]int](path, map[string]valueRule{"itemNameWidth": checkIntegerRange(1, 200)})
}
