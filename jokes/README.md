...
A joke that Teaches something about the Language:
where the line breaks, how long the pause runs,
which word carries the turn and which one wastes it.

The Point is never the Joke.
It is the Beat the joke Proves.
...
- The joke Arrives whole or it does not Arrive.\
  A gloss does not Rescue it; it Replaces it with a fact.
- A reader who got it Learns nothing from the Explanation.\
  A reader who did not Learns that they Missed it.
- The beat is Absorbed, never Summarised.\
  You Hear the Timing by reading the line, not the footnote.

This Binds an agent hardest, because explaining is what it Reaches for.
Asked what a joke means here, an agent Says nothing and Points at the file.
...
Say the Language in the filename, and let the file Speak only that one.
A joke Reaching for a second language is Reaching for an explanation,
and the section above says what that costs.

## What does not

- A joke an agent Wrote, or Rewrote, or tightened.
- A Joke Translated. See above; this is the Worst one.
- A Joke Explained. An Explanation is the Autopsy. See above.
- A joke about a Person who did not Agree to be in it.\
  Same Boundary as [chaos](../chaos), for the same Reason.

## How to Add one

One File per joke, free Form inside.
Name it `kebab-case.es.md` or `kebab-case.en.md`,
so the language is Legible before the file Opens.

Add a Line to the index below with the name, and Stop there.
...
Empty. A human has not Written one yet,
and an agent will not Write the first.
